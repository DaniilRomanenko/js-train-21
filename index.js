...
class Bassist extends Musician {
  // Об'являємо приватні поля  #band0;
  #band;
  // Конструктор приймає три параметри: name, instrument та band
  constructor (name,instrument,band,){
      // виклик конструктора батьківського класу super з двома параметрами name, instrument
...
class Band {
  ...
  constructor(name, members) {
    // Перевіряємо чи функцію було викликано з new
    if (!(this instanceof Band)) {
      throw new Error("Конструктор має бути викликаний з 'new'");
    }

    // Присвоюємо значення вхідного параметру name до приватного поля #name
    this.#name = name;
    // Присвоюємо значення вхідного параметру members до приватного поля #members
    this.#members = members || [];
  }

  // Геттер для #name, що повертає приватну властивість #name
  get name() {
    return this.#name;
  }

  // Геттер для #members, що повертає приватну властивість #members
  get members() {
    return this.#members;
  }
  ...
  addMember(newMember) {
    ...
    if (newMember instanceof Musician) {
      ...
      // Якщо ні, виводимо в консоль повідомлення
      console.log("Новий учасник повинен бути екземпляром класу Musician");
    }
  }

  // Метод playMusic(), за допомогою forEach перебираємо масив і викликаємо метод play() для кожного учасника гурту
  playMusic() {
...
class Performance {
  ...
  constructor(band, location, date) {
    // Присвоюємо значення вхідного параметру band до приватного поля #band
    this.#band = band;
    // Присвоюємо значення вхідного параметру location до приватного поля #location
    this.#location = location;
    // Присвоюємо значення вхідного параметру date до приватного поля #date
    this.#date = date;
  }

  // Геттер для #band, що повертає приватну властивість #band
  get band() {
    return this.#band;
  }

  // Геттер для #location, що повертає приватну властивість #location
  get location() {
    return this.#location;
  }

  // Геттер для #date, що повертає приватну властивість #date
  get date() {
    return this.#date;
  }

  // Метод info(), що виводить рядок в консоль `Гурт ${this.#band.name} виступить в ${this.#location} ${this.#date.toLocaleDateString()}`
  info() {
...
class Concert extends Performance {
  ...
  constructor(band, location, date, ticketPrice) {
    // Викликаємо конструктор базового класу Performance за допомогою super
    super(band, location, date);
    // Присвоюємо значення вхідного параметру ticketPrice до приватного поля #ticketPrice
    this.#ticketPrice = ticketPrice;
  }

  // Геттер для #ticketPrice, що повертає приватну властивість #ticketPrice
  get ticketPrice() {
    return this.#ticketPrice;
  }

  // Сеттер для #ticketPrice, що дозволяє змінити приватну властивість #ticketPrice
  set ticketPrice(newTicketPrice) {
    this.#ticketPrice = newTicketPrice;
  }

  // Метод info(), що виводить рядок в консоль `Гурт ${super.band.name} виступить в ${super.location} ${super.date.toLocaleDateString()}, ціна квитка ${this.#ticketPrice}`
  info() {
...
class Vocalist {
  ...
  // Сеттер для #name, що дозволяє змінити приватну властивість #name
  set name(newName) {
    this.#name = newName;
  }

  // Сеттер для #band, що дозволяє змінити приватну властивість #band
  set band(newBand) {
    this.#band = newBand;
  }

  // Метод info(), який виводить інформацію про вокаліста
  info() {
...
class SongWriter {
  ...
  constructor(songs = []) {
    // Присвоюємо значення вхідного параметру songs до приватного поля #songs
    this.#songs = songs;
  }

  // Геттер для #songs, що повертає приватну властивість #songs
  get songs() {
    return this.#songs;
  }
  ...
  // Метод info(), який виводить інформацію про автора пісень
  info() {
 ...
 * | songs       | ["Yesterday","Hey Jude","Let It Be",]|
 */
const leadSinger = new LeadSinger("Freddie Mercury", "Queen");
band.addMember(leadSinger);
